		...
		compile: function (element, attrs) {
			...
			return function (scope, element, atributos) {
				...
				var controls = angular.element('<div class="pagination-panel">'+
					'<button class="back-button ui teal button" style="margin-top:0px;" ng-click="'+registro.getBeforeNotation()+'()" type="button"'+
					'ng-disabled="'+registro.getCurrentNotation()+'<=0"><i class="left arrow icon"></i></button>'+

					'<small class="indicator small-indicator ">{{'+registro.getCurrentNotation()+'+1}} - {{'+registro.getPagesNotation()+'}}</small>'+

					'<button class="front-button ui teal button" style="margin-top:0px;" ng-click="'+registro.getNextNotation()+'()" type="button"'+
					'ng-disabled="'+registro.getCurrentNotation()+'>=('+registro.getDataLengthNotation()+'/'+registro.getSizeNotation()+')-1"><i class="right arrow icon"></i></button>'+
				'</div>');
